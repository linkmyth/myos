...
page_init(void)
{
	...
	for (i = 0; i < npages; i++) {
		...
		if (i * PGSIZE >= IOPHYSMEM && i * PGSIZE < EXTPHYSMEM) continue;
		if (i * PGSIZE >= EXTPHYSMEM && (i * PGSIZE) < PADDR(nextfree)) continue;
		pages[i].pp_ref = 0;
...
page_free(struct PageInfo *pp)
{
	...
	if (pp->pp_link) {
		panic("page_free: Double free attempts");
	}
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	pd_entry = &pgdir[PDX(la)];
	if (*pd_entry && PTE_P) {
		pgtable = PTE_ADDR(*pd_entry);
	} else {
		if (!create) {
			return NULL;
		}
		...
		++pp->pp_ref;
		pgtable = page2pa(pp);
		pgdir[PDX(la)] = pgtable | PTE_P | PTE_W | PTE_U;
	}
	pt = KADDR(pgtable);
	pt_entry = &pt[PTX(la)];
	return pt_entry;
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	for (i = 0; i * PGSIZE < size; i++) {
		pt_entry = pgdir_walk(pgdir, (const void *)(va + i * PGSIZE), 1);
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	if ((base + size) > (MMIOBASE + 4 * 1024 * 1024)) {
		panic("out of memory");
	}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	for (i = 0; start < stop; start = start + i * PGSIZE) {
		if (!page_lookup(env->env_pgdir, (void *)(start), &pt)) {
			goto error;
		}
		if ((!(*pt & PTE_P))) {
			goto error;
